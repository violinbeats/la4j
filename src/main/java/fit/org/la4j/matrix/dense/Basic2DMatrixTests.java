...
public class Basic2DMatrixTests extends ColumnFixture{
	...
	private int row1, col1, size;//test matrix dimension
	private int mar, mac, mbr, mbc, mcr, mcc, mdr, mdc; //block test
	private double newValue; //set test
...
// behavioral testing
    ...
    public boolean diagnolBasic2DMatrixTest() {
    	...
    	Basic2DMatrix diagnolM = Basic2DMatrix.diagonal(size, diagnolNumber);
    ...
    public boolean swapRowTest() {
    	...
    	double[] newSecondRow = 
    			new double[]{a.getRow(1).get(0),
    					a.getRow(1).get(1)};
    	if (!assertDoubleArrayEquals(newFirstRow, new double[]{1, 9}, eps)) return false;
    	if (!assertDoubleArrayEquals(newSecondRow, new double[]{3, 5}, eps)) return false;
    	return true;
    }
...
//    }
    ...
    public boolean toArrayTest() {
    	double[][] array = a.toArray();
    	if (!assertDoubleArrayEquals(array[0], firstRow, eps)) return false;
    	if (!assertDoubleArrayEquals(array[1], secondRow, eps)) return false;
    	return true;
    }
    
    public boolean blankOfShapeTest() {
    	Basic2DMatrix matrix = (Basic2DMatrix) a.blankOfShape(2, 2);
    	Basic2DMatrix matrixShouldBe = Basic2DMatrix.zero(2, 2);
    	if (!assertMatrixEquals(matrix, matrixShouldBe, eps)) return false;
    	return true;
    }
